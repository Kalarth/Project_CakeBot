...
#fonction deplacement
	...
	def CalcDeplacementVert(self,sens,listePos):
		listePos.append([0,sens + tailleBras,0])

	def CalcDeplacementHor(self,sens,listePos):
		listePos.append([sens + tailleBras,0,0])


	def TestMarqueur(self,listeM,iOutils):
		for i in range(len(listeM-1)):
			if (listeM[i]==iOutils):
				return True
		return False

	def Deplacement(self,tailleBras):

		ListePos =[]
		listeID = detectARUCO(self)
		Test = True

		while(Test == True):
			if(TestMarqueur(self,listeID,iOutils)== True):
				pFinal = (iOutils - idOrigine)
				if(pFinal//10 != 0):
					if(pFinal//10 > 0):
						CalcDeplacementVert(self,1,listPos)
						idOrigine = idOrigine + 10
					if(pFinal//10 < 0):
						CalcDeplacementVert(self,-1,listPos)
						idOrigine = idOrigine - 10
				if(pFinal%10 !=0):
